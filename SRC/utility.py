...
def get_cor(exp_m, cbtype, goi, madf, cort, corf, biotypefilter=False, biotypef=False,docor=False):
    """
    ...
    if biotypefilter:
        if biotypef.columns[0] != 'gene' or biotypef.columns[1] != 'biotype':
            sys.exit("Error: Make sure the column names and the order of the"
    ...
    if madf < 1:
        sys.exit("ERROR: Choose MAD threshold >= 1")
    esub = dat.filter_uninformative(esub, madf)
    mtb=esub['gene'].str.contains(goi).sum()

    if mtb <= 0:
        sys.exit("ERROR: Gene did not pass the MAD threshold. Try setting a"
    ...
    if docor:
        ...
        if cort < 0.6:
            sys.exit("ERROR: Choose correlation threshold >= 0.6, non-negative")
        gcor = gcor[abs(gcor['cor']) > cort]
        if len(gcor.index) < 19:
            sys.exit("ERROR: You don't have enough genes that show correlation"
        ...
        gcor.to_csv("AC.csv", index=False, header=True)
        if cort < 0.6:
            sys.exit("ERROR: Choose correlation threshold >= 0.6")
        gcor = gcor[abs(gcor['cor']) > cort]
        if len(gcor.index) < 19:
            sys.exit("ERROR: You don't have enough genes that show correlation"
            "above your set threshold. Try setting a lower correlation threshold")
...
def save_merged_n_d_ac_fd_co(n,d,ac,fd,co,path,nfile,parameters):
    """
    Save multi-run
    Parameters
    ----------
